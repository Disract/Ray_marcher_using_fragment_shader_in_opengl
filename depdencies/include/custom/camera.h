...
public:
	...
	Camera(float posX, float posY, float posZ, float upX, float upY, float upZ, float yaw, float pitch) : Front(glm::vec3(0.0, 0.0, -1.0f)), MovementSpeed(SPEED), MouseSensitivity(SENSITIVITY), Zoom(ZOOM)
	{
	...
	void ProcessKeyboard(Camera_Movement direction, float deltaTime)
	{
		...
		//movedirection = ort.forward * vert + ort.right * hort;
	...
	void ProcessMouseMovement(float xoffset, float yoffset, GLboolean constrainPitch = true)
	{
		...
		//above a certain pitch dont allow player to move is that what fps camera is ?
		//no
		//camera is independent from movement ?
		if (constrainPitch)
		{
			if (Pitch > 89.0f)
				Pitch = 89.0f;
